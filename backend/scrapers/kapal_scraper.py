...
def _persist_records(records: list[dict], code: str, snapshot_month: str) -> int:
    ...
    for r in records:
        rows.append(dict(
            ...
            # The kapal.dephub.go.id API returns Indonesian field names; the
            # original mapping used English placeholders that never matched.
            call_sign=r.get("HurufPengenal") or r.get("CallSign"),
            ...
            gt=_to_float(r.get("IsiKotor") or r.get("GT")),
            isi_bersih=_to_float(r.get("IsiBersih")),
            imo=r.get("NomorIMO") or r.get("Imo"),
            tahun=r.get("TahunPembuatan") or r.get("Tahun"),
